...
const extractCode = (normalizedObj) => extract(codeRegEx, (match) => ({
    name: "code",
    language: match[2].trim(),
    src: match[3],
}), normalizedObj);
const extractSpoilers = (normalizedObj) => extract(spoilerRegEx, (match) => ({
    name: "spoiler",
    title: match[1],
    content: normalizeMarkdown(match[2]),
}), normalizedObj);
const extractTable = (normalizedObj) => extract(tableRegEx, (match) => ({
    name: "table",
    src: match[0],
}), normalizedObj);
const extractInjections = (normalizedObj) => extract(injectionRegEx, (match) => ({
    name: "injection",
    description: match[1],
    src: match[2],
}), normalizedObj);
const extractGeogebra = (normalizedObj) => extract(geogebraInjectionRegEx, (match) => ({
    name: "geogebra",
    description: match[1],
    src: match[2],
}), normalizedObj);
const extractLinkedImages = (normalizedObj) => extract(linkedImagesRegEx, (match) => ({
    name: "image",
    description: match[1],
    src: match[2],
    title: match[4],
...
const extractImages = (normalizedObj) => extract(imagesRegEx, (match) => ({
    name: "image",
    description: match[1],
    src: match[2],
    title: match[4],
}), normalizedObj);
const extractBlockquote = (normalizedObj) => extract(blockquoteRegEx, (match) => ({
    name: "blockquote",
